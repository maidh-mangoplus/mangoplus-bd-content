...
function bindCardEvents() {
  ...
  document.querySelectorAll('[data-trailer-id]').forEach(btn => btn.addEventListener('click', () => {
    const item = allData.find(d => String(d.id) === btn.dataset.trailerId);
    if (item) openTrailerModal(item.trailerUrl);
  }));
}

// ---- Rendering: Tổng quan (Top 10 + lưới nhóm) ----
...
function renderTongQuan() {
  ...
  document.getElementById('top10-wrap').style.display = top10.length ? '' : 'none';
  ...
  document.getElementById('overview-groups').innerHTML = html ||
...
// ---- Trailer modal (nhúng YouTube/Drive + link mở gốc) ----
...
function openTrailerModal(url) {
  ...
  if (!url) {
    embedArea.innerHTML = '<div class="trailer-embed">Chưa có trailer</div>';
    fallback.style.display = 'none';
  } else if (/youtube\.com|youtu\.be/.test(url)) {
    const id = extractYoutubeId(url);
    embedArea.innerHTML = id
...
function closeTrailerModal() {
  ...
  document.getElementById('trailer-embed-area').innerHTML = ''; // dừng video khi đóng
}
...
function switchViewRaw(view) {
  ...
  if (view === 'chitiet') renderChiTiet(); else renderTongQuan();
}
...
window.addEventListener('resize', () => {
  if (currentView() === 'chitiet') equalizeBodyHeights();
});
function currentView() { return document.querySelector('.view-tabs button.active').dataset.view; }
function refreshCurrentView() { currentView() === 'chitiet' ? renderChiTiet() : renderTongQuan(); }

// ---- Khởi tạo: gọi dữ liệu thật từ Apps Script ----
async function init() {
  try {
    const res = await fetch(API_URL);
    if (!res.ok) throw new Error('Network error: ' + res.status);
    const raw = await res.json();
    allData = raw.map((d, i) => Object.assign({ id: i + 1 }, d));
    renderTongQuan();
    updateNavButtons();
  } catch (err) {
    document.getElementById('overview-groups').innerHTML =
